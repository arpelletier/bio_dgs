...
class Tester(object):
    ...
    def build(self, graphtype, save_path = 'this-model.ckpt', data_save_path = 'this-data.bin', other_save_path = 'model-weight'):
        print(save_path, data_save_path, other_save_path)
    ...
    def projection_vert(self, e_vert, source, activation=True):
        ...
        #return np.add(np.dot(vec_e, self.mat), self._b)
        if activation:
            print(type(vec_e_vert))
            print(vec_e_vert.shape)
            print(vec_e_vert.shape)
